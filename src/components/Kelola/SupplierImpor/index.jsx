...
const SupplierImpor = () => {
  ...
  const fetchSupplier = async () => {
    try {
      const { data, error } = await supabase.from("supp_impor").select("*");
  ...
  const handleSubmit = async (values) => {
    console.log(values);
    try {
      const { data, error } = await supabase.from("supp_impor").insert([
        {
          id_impor: await supabase
            .from("supp_impor")
            .select("id_impor", { count: "exact", head: true })
            .then((r) => r.count + 1),
    ...
    <div className="max-w-screen-xl">
      <div className="grid gap-4">
        <Drawer
          ...
          extra={<p className="text-lg font-bold">Tambah Supplier Impor</p>}
          footer={
            <div
              style={{
                display: "flex",
